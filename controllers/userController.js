    ...
    try {
      ...
      connection.query(
        ...
        (err, result1) => {
          if (result1) {
            connection.query(
              `SELECT name,email,dob,mobile,gender,alternatePhone,lastActive FROM users WHERE NOT email=?`,
              [req.email.email],
              (err, result2) => {
                if (result2) {
...
//     console.log(start);
//     connection.query(`ALTER TABLE bp ADD COLUMN ${start} varchar(255)`, (err, result) => {
//       if (result) {
// 		res.send(result)
//       }
// 	  else
